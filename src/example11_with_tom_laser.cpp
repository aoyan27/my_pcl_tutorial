...
nav_msgs::OccupancyGrid tom_laser_local_map;
...
void TomLaserLocalMap_callback(nav_msgs::OccupancyGrid msg){
	tom_laser_local_map = msg;
	// real_local_map.header.frame_id = "/obstacle_local_map";
	cout<<"tom_laser_local_map.header.frame_id : "<<tom_laser_local_map.header.frame_id<<endl;
	sub_tom_laser_flag = true;
}
...
void tom_laser2obstacle(nav_msgs::OccupancyGrid *map){
	geometry_msgs::Point real_point, obstacle_point;
	nav_msgs::OccupancyGrid out;
	out.header.frame_id = "/velodyne";
	out.header.stamp = ros::Time::now();
...
void create_local_map(nav_msgs::OccupancyGrid real_local, nav_msgs::OccupancyGrid tom_laser_local, nav_msgs::OccupancyGrid static_local, nav_msgs::OccupancyGrid *map){
	...
	for(int i=0; i<map->data.size(); i++){
		...
		if(static_local.data[i] != 0 || real_local.data[i] != 0  || tom_laser_local.data[i] != 0){
			map->data[i] = 100;
			// cout<<"map->data["<<i<<"] : "<<map->data[i]<<endl;
		}	
	}
...
int main(int argc, char** argv){
	...
	ros::Subscriber sub2 = n.subscribe("/local_map_real/tom_laser/expand", 1, TomLaserLocalMap_callback);
	ros::Subscriber sub3 = n.subscribe("/local_map_static/expand", 1, StaticLocalMap_callback);
	...
	while(ros::ok()){
		if(sub_real_flag && sub_static_flag && sub_tom_laser_flag){
			real2obstacle(&real_local_map);
			tom_laser2obstacle(&tom_laser_local_map);
			static2obstacle(&static_local_map);
			create_local_map(real_local_map, tom_laser_local_map, static_local_map, &local_map);
			sub_real_flag = false;
			sub_tom_laser_flag = false;
