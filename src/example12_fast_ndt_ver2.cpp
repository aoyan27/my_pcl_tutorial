...
int main(int argc, char** argv){
	...
	while(ros::ok()){
		...
		if(real_sub_flag && static_sub_flag && odom_sub_flag && tiny_sub_flag){
			...
			voxel_filter.setInputCloud(real_points_rc);
			...
			voxel_filter_target.setInputCloud(static_points_rc);
			...
			if(fabs(dyaw) < dyaw_threshold){
				...
				odom.pose.pose.position.x += l * sin(theta);
				odom.pose.pose.position.y += l * cos(theta);
				// odom.pose.pose.position.x += ndt.getFinalTransformation()(1, 3);
