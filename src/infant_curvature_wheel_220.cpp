...
void pcCallback(const sensor_msgs::PointCloudConstPtr& msg)
{
        ...
        if(save_flag && pc_count % SAVE_CURVATURE == 0){
                ...
                for(size_t i=0;i<pcl_curvature.points.size();i++){
                        double oval = ( (pcl_curvature.points[i].x) * (pcl_curvature.points[i].x))/(0.3*0.3) + (pcl_curvature.points[i].y * pcl_curvature.points[i].y)/(0.50 * 0.50);
                        ...
                        if( ( -1.25 < pcl_curvature.points[i].z) && (pcl_curvature.points[i].z <0.20) && (oval > 1) && (pcl_curvature.points[i].curvature > 0.07))
                        {
...
{
        ros::init(argc,argv,"infant_curvature_wheel_220");
        ros::NodeHandle n;
        ...
        pub_obstacle = n.advertise<sensor_msgs::PointCloud2>("point_cloud2/curvature_obstacle_220",10);
        pub_empty = n.advertise<sensor_msgs::PointCloud2>("point_cloud2/curvature_empty_220",10);
