...
// const float curvature_min = 0.30;// for ikuta
// const float curvature_max = 0.35;// for ikuta


const int cell_filter = 1;//for tsukuba
const int color_filter = 100; // for tsukuba
const float curvature_min = 0.2;// for tsukuba
const float curvature_max = 0.25;// for tsukuba
...
void remove_point_by_heght(pcl::PointCloud<pcl::PointSurfel>::Ptr cloud, nav_msgs::OccupancyGrid *map){
	...
	cout<<"cloudpoints.size() : "<<cloud->points.size()<<endl;
	...
	// for(size_t i = 0; i < cloud->points.size(); i++){
		...
		// int num = x + y * map->info.width;
		// // // cout<<"num : "<<num<<endl;
		// if(data[num] < cloud->points[i].z && cloud->points[i].z <= (data[num] + max_height)){
			// // cout<<"num : "<<num<<endl;
			// if(curvature_min <= cloud->points[i].curvature && cloud->points[i].curvature <= curvature_max){
				// pcl::PointXYZ temp_point;
...
void create_obstacle_map(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, nav_msgs::OccupancyGrid *map){
	...
	for( size_t i=0; i < cloud->points.size(); i++ ){
		...
		if(count[num] > cell_filter){
			obs_point.x = cloud->points[i].x;
			obs_point.y = cloud->points[i].y;
			obs_point.z = cloud->points[i].z;
			obs_cloud->push_back(obs_point);				
		}
	...
	for(int i = 0;i<count.size(); i++){
		if(count[i] > cell_filter){
			...
			obstacle_point.z = 0;
			// obs_point.x = (i % map->info.width) * R + map->info.origin.position.x;
			// obs_point.y = int(i / map->info.width) * R + map->info.origin.position.y;
			// obs_point.z = 0;
			// obstacle_point.x = cloud->points[i].x;
			// obstacle_point.y = cloud->points[i].y;
			// obstacle_point.z = 0;
	...
	// pcl::io::savePCDFile("obs_cloud_data_final_ikuta_final.pcd", *obs_cloud);
	// pcl::io::savePCDFile("obs_cloud_data_curvature_filtered.pcd", *filtered_cloud);
	// pcl::io::savePCDFile("obs_cloud_data_final_tsukuba.pcd", *obs_cloud);
	// pcl::io::savePCDFile("obs_cloud_data_final_din.pcd", *obs_cloud);
	pcl::io::savePCDFile("obs_cloud_data_final_d_kan_velo90test.pcd", *obs_cloud);
...
int main (int argc, char** argv){
	...
	// string filename = "/home/amsl/obs_map_final_ikuta.pcd";
	// string filename = "/home/amsl/obs_map_final_ikuta_final.pcd";
	// string filename = "/home/amsl/obs_map_new_ikuta.pcd";
	// string filename = "/home/amsl/obs_map_new_ikuta2.pcd";
	// string filename = "/home/amsl/obs_map_ohshimizu.pcd";
	// string filename = "/home/amsl/map_tsukuba_all.pcd";
	// string filename = "/home/amsl/map_tsukuba_all2.pcd";
	// string filename = "/home/amsl/obs_map_final_tsukuba.pcd";
	// string filename = "/home/amsl/map_in_dkan.pcd";
	string filename = "/home/amsl/d_kan_velo90test.pcd";
	if(pcl::io::loadPCDFile (filename, *cloud_data) == -1){
		PCL_ERROR("Couldn't read file '%s'", filename.c_str());
		return -1;
	}

	cout<<"proglam start!!!!!!"<<endl;	
	...
	cout<<"remove_point faunction start!!!!!!!!"<<endl;
	...
	cout<<"create_obstacle_map function start!!!!!!!!!!!"<<endl;
