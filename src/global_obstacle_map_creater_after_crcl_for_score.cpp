...
pcl::PointCloud<pcl::PointSurfel>::Ptr cloud_obs_data (new pcl::PointCloud<pcl::PointSurfel>);
...
pcl::PointCloud<pcl::PointXYZ>::Ptr temp_road_cloud (new pcl::PointCloud<pcl::PointXYZ>);
...
nav_msgs::OccupancyGrid obstacle_road_map;
nav_msgs::GridCells obstacle_cell;

nav_msgs::OccupancyGrid local_map;
nav_msgs::OccupancyGrid local_map_expand;
nav_msgs::OccupancyGrid local_road_map;
...
void remove_point_by_heght(pcl::PointCloud<pcl::PointSurfel>::Ptr cloud, nav_msgs::OccupancyGrid *map){
		...
		for(size_t i = 0; i < cloud->points.size(); i++){
			...
			if(cloud->points[i].z < (data[num] + min_height)){
				...
				temp_road_cloud->push_back(temp_point);
			}
		...
		pcl::io::savePCDFile("obs_road_data.pcd", *temp_road_cloud);
		cout<<"PCD File save!!!"<<endl;

		// cout<<"temp_road_cloud.size() : "<<temp_road_cloud->size()<<endl;
		// pcl::VoxelGrid<pcl::PointXYZ> voxel_filter;
		// voxel_filter.setLeafSize(0.1, 0.1, 0.1);
		// voxel_filter.setInputCloud(temp_road_cloud);
...
void create_obstacle_road_map(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, nav_msgs::OccupancyGrid *map){
	geometry_msgs::Point obstacle_point;
...
void create_local_road_map(float x, float y, float theta, pcl::PointCloud<pcl::PointXYZ>::Ptr road_cloud , nav_msgs::OccupancyGrid* loc_map){
	nav_msgs::OccupancyGrid out;
	...
	for(size_t i = 0; i < road_cloud->points.size(); i++){
		// cout<<"obs_grid.cells.size() : "<<obs_grid.cells.size()<<endl;
		// cout<<"i : "<<i<<endl;
		int xi = int((rotation_x(road_cloud->points[i].x-x, road_cloud->points[i].y-y, -1*theta) - out.info.origin.position.x) / R);
		// int xi = int((rotation_x(obstacle_points.points[i].x-x, obstacle_points.points[i].y-y, theta) - out.info.origin.position.x) / R);
		// cout<<"xi : "<<xi<<endl;
		int yi = int((rotation_y(road_cloud->points[i].x-x, road_cloud->points[i].y-y, -1*theta) - out.info.origin.position.y) / R);
		// int yi = int((rotation_y(obstacle_points.points[i].x-x, obstacle_points.points[i].y-y, theta) - out.info.origin.position.y) / R);
		// cout<<"yi : "<<yi<<endl;
		if((0 <= xi && xi < local_W/R) && (0 <= yi && yi < local_H/R)){
			int num = xi + yi * out.info.width;
...
int main (int argc, char** argv){
	...
	string filename_obs = "/home/amsl/obs_map.pcd";
	...
	if(pcl::io::loadPCDFile (filename_obs, *cloud_obs_data) == -1){
		PCL_ERROR("Couldn't read file '%s'", filename_obs.c_str());
		return -1;
	}
	...
	ros::Subscriber sub12 = n.subscribe("/ekf_DgaussAndNDT", 1 , ndt_odom_callback);
	...
	ros::Publisher pub2_road = n.advertise<nav_msgs::OccupancyGrid>("/ob_road_map", 1);
	...
	ros::Publisher pub3_road = n.advertise<nav_msgs::OccupancyGrid>("/local_map_static/road", 1);
	...
	obstacle_road_map = obstacle_map;
	...
	local_road_map = local_map;
	// remove_point_by_heght(cloud_data);
	remove_point_by_heght(cloud_obs_data, &obstacle_map);
	PointXYZ_to_PointCloud2(&cloud_data, &cloud2);

	create_obstacle_map(cloud_data, &obstacle_map);
	create_obstacle_road_map(temp_road_cloud, &obstacle_road_map);
	...
	while(ros::ok()){
		if(pub_flag){
			...
			obstacle_road_map.header.stamp = ros::Time::now();
			pub2_road.publish(obstacle_road_map);
		...
		create_local_map(ndt_x, ndt_y, ndt_theta, obstacle_cell, &local_map);
		create_local_map(ndt_x, ndt_y, ndt_theta, obstacle_cell, &local_map_expand);
		
		// create_local_road_map(ndt_x, ndt_y, ndt_theta, temp_road_cloud, &local_road_map);
		...
		pub3_road.publish(local_road_map);
