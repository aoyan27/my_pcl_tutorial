...
void constructFullClouds(const CloudT& pcl_in, CloudT&pcl_obstacle, CloudT& pcl_empty, const CloudT& pcl_velodyne)
{
        ...
        for(unsigned i=0;i<npoints;i++)
        {
                ...
                double oval = (pcl_in.points[i].x * pcl_in.points[i].x)/(0.075 * 0.075) + (pcl_in.points[i].y * pcl_in.points[i].y)/(0.2 * 0.2);

                if( x >= 0 && x < grid_dimentions && y >= 0 && y < grid_dimentions && init[x][y])
                {
                        if(( max[x][y]-min[x][y] > height_threshold) && ( max[x][y]-min[x][y] < max_height) && (pcl_in.points[i].z < 0.4)  && (pcl_in.points[i].x >= 0.1) && (pcl_in.points[i].z >= -0.85 ))
                        {
                                pcl::PointXYZ p;
...
{
        ros::init(argc,argv,"infant_height_map_odom_222");
        ros::NodeHandle n;
