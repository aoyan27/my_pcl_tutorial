...
void pcCallback(const sensor_msgs::PointCloudConstPtr& msg)
{
        ...
        if(save_flag && pc_count % SAVE_CURVATURE == 0){
                ...
                for(size_t i=0;i<pcl_curvature.points.size();i++){
                        ...
                        if( ( -1.12 < pcl_curvature.points[i].z) && (pcl_curvature.points[i].z <0.20) && (oval > 1) && (pcl_curvature.points[i].curvature > 0.07))
                        {
...
{
        ros::init(argc,argv,"infant_curvature_wheel_221");
        ros::NodeHandle n;
