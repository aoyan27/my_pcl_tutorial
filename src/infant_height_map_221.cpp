...
void constructFullClouds(const CloudT& pcl_in, CloudT&pcl_obstacle, CloudT& pcl_empty)
{
        ...
        for(unsigned i=0;i<npoints;i++)
        {
                ...
                int x = ((grid_dimentions/2)+pcl_in.points[i].x/cell_size);
                int y = ((grid_dimentions/2)+pcl_in.points[i].y/cell_size);
                double r = sqrt((pcl_in.points[i].x*pcl_in.points[i].x) + (pcl_in.points[i].y*pcl_in.points[i].y));
                double oval = ((pcl_in.points[i].x+0.2) * (pcl_in.points[i].x+0.20))/(0.40 * 0.40) + (pcl_in.points[i].y * pcl_in.points[i].y)/(0.50 * 0.50);
                if(pc_count%40==0 && i==540){
                        cout<<"range:"<<r;
                        cout<<" oval:"<<oval;
                        cout<<" x:"<<pcl_in.points[i].x;
                        cout<<" y:"<<pcl_in.points[i].y<<endl;
                }

                if( x >= 0 && x < grid_dimentions && y >= 0 && y < grid_dimentions && init[x][y])
                {
                        //if(( max[x][y]-min[x][y] > height_threshold) && ( max[x][y]-min[x][y] < max_height) && (-0.85 < pcl_in.points[i].z) && (pcl_in.points[i].z < 0.4) &&  (r>0.2 ))//&& 0.1 < pcl_in.points[i].x))
                        if( ( max[x][y]-min[x][y] > height_threshold) && ( max[x][y]-min[x][y] < max_height) && (-0.85 < pcl_in.points[i].z) && (pcl_in.points[i].z < 0.4) && (oval > 1.0))
                        {
                                pcl::PointXYZ p;
...
{
        ros::init(argc,argv,"infant_height_map_221");
        ros::NodeHandle n;
