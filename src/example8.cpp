...
void create_obstacle_map(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, nav_msgs::OccupancyGrid *map){
	...
	geometry_msgs::Point32 obstacle_point32;
	...
	for(int i = 0;i<count.size(); i++){
		if(count[i] > 1){
			...
			obstacle_point32.x = (i % map->info.width) * R + map->info.origin.position.x;
			obstacle_point32.y = int(i / map->info.width) * R + map->info.origin.position.y;
			obstacle_point32.z = 0;
			
			obstacle_points.points.push_back(obstacle_point32);
...
void expand_circle(int x0, int y0, int radius, nav_msgs::OccupancyGrid *map){
	...
	int err = 0;
		...
		while(x >= y){
			...
			int num6 = (x0-x)+(y0-y)*map->info.width;
...
void tf_map2local_map(float x, float y, float theta, tf::TransformBroadcaster br){
...
int main (int argc, char** argv){
	...
	//初期位置合わせ(2015-12-22-14-28-54.bagとmap_Dkan_test.pcdの組み合わせ。)///////////
	float x_init = 1.5;
	float y_init = -28.0;
	...
	while(ros::ok()){
		...
		if(sub_flag){
			x = -1*odom.pose.pose.position.x + x_init;
			// x = odom.pose.pose.position.y + y_init;
			// cout<<"x : "<<x<<endl;
			y = -1*odom.pose.pose.position.y + y_init;
			// y = odom.pose.pose.position.x + x_init;
			// cout<<"y : "<<y<<endl;
			theta = tf::getYaw(odom.pose.pose.orientation) + theta_init;
			// cout<<"theta : "<<theta<<endl;
			sub_flag =false;
		}
		cout<<"x : "<<x<<endl;
