...
void constructFullClouds(const CloudT& pcl_in, CloudT&pcl_obstacle, CloudT& pcl_empty, const CloudT& pcl_velodyne)
{
        ...
        for(unsigned i=0;i<npoints;i++)
        {
                int x = ((grid_dimentions/2)+pcl_in.points[i].x/cell_size);
                int y = ((grid_dimentions/2)+pcl_in.points[i].y/cell_size);
                double oval = ((pcl_in.points[i].x+0.10) * (pcl_in.points[i].x+0.10))/(0.45 * 0.45) + (pcl_in.points[i].y * pcl_in.points[i].y)/(0.70 * 0.70);

                if( x >= 0 && x < grid_dimentions && y >= 0 && y < grid_dimentions && init[x][y])
                {
                        if(( max[x][y]-min[x][y] > height_threshold) && ( max[x][y]-min[x][y] < max_height) && (pcl_in.points[i].z < 0.4)  && (pcl_in.points[i].x >= 0.1) && (pcl_in.points[i].z >= -0.60 ) && (oval>1))
                        {
                                pcl::PointXYZ p;
...
{
        ros::init(argc,argv,"infant_height_map_wheel_222");
        ros::NodeHandle n;
