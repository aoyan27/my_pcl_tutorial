...
//		int xi = int((rotation_x(obs_grid.cells[i].x-x, obs_grid.cells[i].y-y, -1*(theta)) - out.info.origin.position.x) / 0.1);
//		int yi = int((rotation_y(obs_grid.cells[i].x-x, obs_grid.cells[i].y-y, -1*(theta)) - out.info.origin.position.y) / 0.1);
	...
	sensor_msgs::PointCloud2 stat_cloud_ros;
	PointXYZ_to_PointCloud2(&static_points_rc, &stat_cloud_ros);
	stat_cloud_ros.header = out.header;
	_rc_stat_pub.publish(stat_cloud_ros);
	
	//PC2GridMap(*static_points_rc, out_rc);
...
/* for debug */
	sensor_msgs::PointCloud2 real_cloud_ros;
	PointXYZ_to_PointCloud2(&real_points_rc, &real_cloud_ros);
	real_cloud_ros.header = map.header;
	_rc_real_pub.publish(real_cloud_ros);

	//PC2GridMap(*real_points_rc, realtime_map);
	//PC2GridMap(*real_points_rc);
...
void gaussSphereCallback(nav_msgs::Odometry msg){
	score_flag = true;
	if(realtime_flag && gauss_pose_flag){
		score_msg.data = calcScore();
		printf("Dgauss score : %f\n", score_msg.data);
...
void DgaussCallback(nav_msgs::Odometry msg){
...
//	realtime_map.header.frame_id = "matching_base_link";
...
//	local_map.header.frame_id = "/matching_base_link";
...
int main(int argc, char** argv){
	ros::init(argc, argv, "Dgauss_score");
	ros::NodeHandle n;
	
	ros::Subscriber flag_sub = n.subscribe("/gauss_sphere/pose", 1, gaussSphereCallback);
	ros::Subscriber real_sub = n.subscribe("/local_map_real", 1, realtimeMapCallback);
	ros::Subscriber gauss_sub = n.subscribe("/lcl", 1, DgaussCallback);
	
	_rc_real_pub = n.advertise<sensor_msgs::PointCloud2>("debug_realtime_pc", 1);
	_rc_stat_pub = n.advertise<sensor_msgs::PointCloud2>("debug_static_pc", 1);
	real_pub = n.advertise<nav_msgs::OccupancyGrid>("/real_map_debug", 1);
	stat_pub = n.advertise<nav_msgs::OccupancyGrid>("/static_map_debug", 1);
	...
	score_pub = n.advertise<std_msgs::Float32>("/DgaussScore", 1);
