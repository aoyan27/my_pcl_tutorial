...
void pcCallback(const sensor_msgs::PointCloudConstPtr& msg)
{
    ...
    if(save_flag && pc_count % SAVE_CURVATURE == 0){
            ...
            for(size_t i=0;i<pcl_curvature.points.size();i++){
                    ...
                    if( ( -0.75 < pcl_curvature.points[i].z) && (pcl_curvature.points[i].z <0.40) && (oval > 1) && (pcl_curvature.points[i].curvature > 0.07))
                    {
...
{
	ros::init(argc,argv,"infant_curvature_220_new");
	ros::NodeHandle n;
