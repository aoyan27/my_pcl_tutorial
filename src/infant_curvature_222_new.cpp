...
void pcCallback(const sensor_msgs::PointCloudConstPtr& msg)
{
    ...
    if(save_flag && pc_count % SAVE_CURVATURE == 0){
            ...
            for(size_t i=0;i<pcl_curvature.points.size();i++){
                    double oval = ( (pcl_curvature.points[i].x+0.4) * (pcl_curvature.points[i].x+0.4))/(0.6*0.6) + (pcl_curvature.points[i].y * pcl_curvature.points[i].y)/(0.50 * 0.50);
                    ...
                    if( ( -0.75 < pcl_curvature.points[i].z) && (pcl_curvature.points[i].z <0.40) && (oval > 1) && (pcl_curvature.points[i].curvature > 0.06))
                    {
...
{
	ros::init(argc,argv,"infant_curvature_222_new");
	ros::NodeHandle n;
